...
GlobalType=function(catKey,divId,conf){
	...
	this.loadGlobalTree=function(){
		...
		var url=__ctx + "/cloud/saas/bpm/globalType/getByCatKey.ht";
		if(conf.url){
			url=conf.url;
		}
	...
	this.delNode=function (){
		...
		if(typeId!=0){
			var url=__ctx +'/cloud/saas/bpm/globalType/del.ht';
	...
	this.openGlobalTypeDlg=function(isAdd,isPrivate){
		...
		var url=__ctx +'/cloud/saas/bpm/globalType/dialog.ht';
		if(isAdd){
			if(isRoot){
	...
	this.sortNode=function(){
		var typeId=_self.currentNode.typeId;
		var url=__ctx +'/cloud/saas/bpm/globalType/sortList.ht?parentId='+typeId;
