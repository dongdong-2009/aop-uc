...
function batchPass(state){
	...
	if(row==null||row.length==0){
 		$.ligerMessageBox.alert('提示信息','请选择审核的企业');
		return false;
 	}
	...
	//alert(sysOrgInfoIds);
	var url="${ctx}/tenant/batchPass.ht?sysOrgInfoIds="+sysOrgInfoIds+"&userId="+${userId}+"&fromSysId="+${fromSysId};
	 $.ajax({
			type : 'post',
			dataType : 'json',
			async : false,
			...
			success : function(dics){
				if(dics){
					window.location.href="${ctx}/tenant/auditSubsystem.ht?userId="+${userId}+"&fromSysId="+${fromSysId};
				}else{
					layer.alert("审核失败！");
				}
			}
	...
	layer.confirm('确定驳回吗？', {
		   btn: ['确定','取消'] //按钮
			 ...
			 $.ajax({
					...
					url : "${ctx}/tenant/batchReject.ht?sysOrgInfoIds="+sysOrgInfoIds+"&fromSysId="+${fromSysId},
					success : function(dics){
						if(dics){
							window.location.href="${ctx}/tenant/auditSubsystem.ht?userId="+${userId}+"&fromSysId="+${fromSysId};
						}else{
	...
	<div class="panel">
		<div class="panel-top">
			<div class="tbar-title">
				<span class="tbar-label">企业管理列表</span>
			</div>
			...
			<div class="panel-search">
				<form id="searchForm" method="post" action="auditSubsystem.ht?userId=${userId}&fromSysId=${fromSysId}">
		...
		<div class="panel-body">
			    ...
			    <display:table name="tenantList" id="InfoItem" requestURI="auditSubsystem.ht?userId=${userId}&fromSysId=${fromSysId}" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
					...
					<display:column  title="注册时间" sortable="true" sortName="createtime">
						<fmt:formatDate value="${not empty InfoItem.createtime ?InfoItem.createtime:InfoItem.registertime}" pattern="yyyy-MM-dd"/>
					</display:column>
					<display:column  title="编辑企业" sortable="true" sortName="name">
						<a href="${ctx}/tenant/baseInfoSubsystem.ht?sysOrgInfoId=${InfoItem.sysOrgInfoId}" class="link update">编辑企业</a>
					</display:column>
