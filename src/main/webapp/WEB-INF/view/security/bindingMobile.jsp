	 ...
	 <body>
		...
		<div class="changeMobile_intro">
		     <form id="loginForm" method="post" >
				    ...
				    <label class="findPwd_mainCon_tit">手机号码：</label>
				    ...
				    <input type="text" id="mobile" name="mobile" class="findPwd_mainCon_con" value="${user.mobile }" />
				    </c:if>
				    ...
				    <!-- <input type="text" id="mobile" name="mobile" maxlength="11" class="findPwd_mainCon_con" required mobile="true" data-msg-mobile="请输入正确手机号码" /> -->
				    <div style="font-size:14px;margin-left: 180px;margin-top: 10px;"><img src="${ctx}/pages/images/warn.png" style="margin: -10px 0px;"></img>请输入常用手机号码，以便开展业务</div>
				  </div> 
				 <div class="form-list1">
				   ...
				   <input type="text"  style="width: 180px"  id="verifycode" name="verifycode" maxlength="4" required class="findPwd_mainCon_con"  /> &nbsp;&nbsp;<input type="button" style="height: 38px;border-radius:5px;padding:0 5px;cursor: pointer;" name="verifyBtn" id="verifyBtn" value="发送验证码" onclick="verify();"/>
					<!-- <div style="font-size:14px;margin-left: 180px;">此项为必填项</div> -->
				 </div>
				  <div class="form-list" > 
				...
				success : function(resultMessage) {
					if(resultMessage && resultMessage.indexOf("成功") > 0){
						layer.alert("手机验证码校验成功",function(){
							window.location.href="${ctx}/security/bindingMobileSuccess.ht?mobile="+mobile;
						});
						//window.location.href="${ctx}/security/bindingMobileSuccess.ht?mobile="+mobile;
					}else{
