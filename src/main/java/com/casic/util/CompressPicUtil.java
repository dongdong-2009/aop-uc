...
public class CompressPicUtil {
	...
	public String compressPic() {
		try {
			// 获得源文件
			file = file==null?new File(inputDir + inputFileName):file;
			if (!file.exists()) {
				return "";
			}
	...
	public String compressPic(File file, File outputFile, int width, int height,boolean gp) {
		// 图片文件
		this.file = file;
		// 输出图路径
		this.outputDir = outputFile.getParent();
		// 输出图文件名
	...
	public String compressPic(File file, String outputPath,int width, int height,boolean gp) {
		if(outputPath==null)
			return null;
		...
		String outputDir = outputPath.substring(0, i);
		String outputFileName = outputPath.substring(i + 1);
		this.outputDir = outputDir;
		// 输出图文件名
		this.outputFileName = outputFileName;
		// 设置图片长宽
